...
- **Xcode 27 (2026-09-16, build 17):** the toolchain now *errors* on any pod target with a deployment target below iOS 15, and five resource-bundle targets (RevenueCat, PurchasesHybridCommon, SuperwallKit, AsyncStorage, react-native-view-shot) still declare 9.0–13.0. `plugins/with-pod-deployment-target.js` (registered in `app.json → plugins`) adds a Podfile `post_install` that lifts them to the app's own target. Also seen once: the `ExpoModulesJSI` Swift compile dying with "failed with exit code 0 but produced no further output" and no diagnostics — a rerun of the same archive command succeeded, so retry before digging (expo/expo#50067 tracks the Swift 6.2+ issues in that package). `ExportOptions.plist` has `destination: upload`, so `xcodebuild -exportArchive` with `-authenticationKeyPath/-authenticationKeyID/-authenticationKeyIssuerID` uploads directly; altool is not needed.
