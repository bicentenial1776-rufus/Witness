...
2. **The three.js version conflict is now fully specified, and the mandate is r128.** Both demos load r128 from cdnjs, and `FSV_DEVELOPER_PROMPT_2026-08-04.md` §42 is explicit — "three.js stays (r128 across everything); a version bump is a deliberate, gated task of its own, never a drive-by" — reinforced by its do-not list ("do not upgrade three.js in passing"). `apps/streetview-lab` pins `three@^0.178.0` and `engine.ts` runs on it, but that lab predates rev. 3. **The lab is the deviation, not the standard.** Resolving it is a gated decision of its own — either the lab comes back to r128, or r178 is adopted deliberately with Greg's sign-off. It must not be settled by whichever code gets written next.
