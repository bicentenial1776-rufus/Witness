...
- **Web ≥900px is the Broadsheet carrier**, gated by `useBroadsheet()`: pages wrap in `PageShell` + `Masthead` (`src/components/broadsheet/`), the left rail mirrors the five phone tabs (Home · Tree · Explore · Map · Nearby — same IA as the phone, per docs/phone-ia-design-brief.md), and the phone tab bar takes over below 900px. Detail-on-wide is a right-slide drawer, rendered outside PageShell.
