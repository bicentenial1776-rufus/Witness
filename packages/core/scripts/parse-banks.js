...
const PROSE_WORDS = /\b(Master|tons?|sailed|arrived|passengers?|ordnance|brought|vessel|ships?|fleet|colonists?|Gravesend|Southampton|Weymouth|Bristol|cattle|voyage|records?|probably|supra|ibid)\b/i;
...
export function readName(line) {
    if (!/^[A-Z]/.test(line))
        return null;
    if (line === line.toUpperCase())
        return null; // headings / furniture
    if (PROSE_WORDS.test(line))
        return null;
    const tokens = line.split(/\s+/);
    let i = 0;
    const name = [];
    if (TITLES.test(tokens[0] ?? ''))
        i += 1;
    ...
    if (name.length < 2)
        return null;
    const suffixes = [];
    while (name.length > 2 && SUFFIX.test(name[name.length - 1] ?? ''))
        suffixes.push(name.pop());
...
function main() {
    ...
    for (const v of kept) {
        ...
        const csv = ['given,surname,notes,source']
            .concat(rows.map((r) => [r.given, r.surname, r.notes, r.source]
...
if (process.argv[1]?.endsWith('parse-banks.ts'))
    main();
