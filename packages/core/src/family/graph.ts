 ...
 * be built from a freshly parsed GEDCOM (tests, import-time) or from the
 * database rows the app already has (runtime).
 */
...
function emptyPerson(id: string): GraphPerson {
  return {
    id,
    name: 'Unknown',
    sex: 'U',
...
function wire(people: Map<string, GraphPerson>, families: FamilyLink[]): FamilyGraph {
